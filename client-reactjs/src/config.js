...
const envSchema = joi
  .object({
    NODE_ENV: joi
      .string()
      .valid(["development", "production", "test", "provision", "prod", "dev"])
      .default("dev")
      .required(),
    LOGGER_LEVEL: joi
      .string()
    ...
    LOGGER_ENABLED: joi
      ...
      .falsy("false")
      .default(true),
    API_ROOT: joi.string().uri().default("http://localhost:8080/api/v1/"),
  })
  .unknown()
  .required();
...
const config = {
  ...
  isDevelopment:
    envVars.NODE_ENV === "development" || envVars.NODE_ENV === "dev",
  logger: {
