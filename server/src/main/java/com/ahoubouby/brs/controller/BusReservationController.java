...
import com.ahoubouby.brs.model.request.BookTicketRequest;
import com.ahoubouby.brs.model.request.GetTripSchedulesRequest;
import com.ahoubouby.brs.service.BusReservationService;
import com.ahoubouby.brs.service.UserService;
import com.ahoubouby.brs.utils.DateUtils;
...
public class BusReservationController {
    ...
    public Response getTripSchedules(@RequestBody @Valid GetTripSchedulesRequest getTripSchedulesRequest) {
        ...
        return Response.notFound()
                .setErrors(String.format("No trips between source stop - '%s' and destination stop - '%s' on date - '%s' " +
        ...
        if (userDto.isPresent()) {
            ...
            if (tripDto.isPresent()) {
                Optional<TripScheduleDto> tripScheduleDto = Optional
                        .ofNullable(busReservationService.getTripSchedule(tripDto.get(),
                                DateUtils.formattedDate(bookTicketRequest.getTripDate()),
                                true));
